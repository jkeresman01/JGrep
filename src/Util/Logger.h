...
#define JGREP_DEBUG(message) LOG("DEBUG", message)
#define JGREP_INFO(message) LOG("INFO", message)
#define JGREP_WARN(message) LOG("WARN", message)
#define JGREP_ERROR(message) LOG("ERROR", message)
...
class Logger
{
  public:
    ...
    static void log(const std::string &severity, const std::string &message, const char *file,
                    uint32_t lineNumber)
    {
        std::ofstream logfile("C:\\Users\\josip\\aaaaa\\JGrep\\log.txt", std::ios::app);
